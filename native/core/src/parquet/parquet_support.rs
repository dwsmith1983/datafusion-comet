...
use crate::parquet::schema_adapter::{java_lowercase, JvmCaseTables};
...
pub struct SparkParquetOptions {
    ...
    /// Case tables generated by the planning JVM (`NativeScanCommon.jvm_*` fields), present
    /// whenever `case_sensitive` is false on the plan-based scan paths. They let every
    /// case-insensitive name comparison reproduce that JVM's `toLowerCase(Locale.ROOT)`
    /// exactly; when absent, matching falls back to `str::to_lowercase` (see
    /// `java_lowercase` in `schema_adapter.rs`).
    pub jvm_case_tables: Option<Arc<JvmCaseTables>>,
}
...
fn parquet_convert_struct_to_struct(
    ...
    match (from_type, to_type) {
        (DataType::Struct(from_fields), DataType::Struct(to_fields)) => {
            ...
            // ID lookups keep EVERY index sharing an ID: Spark's `matchIdField` raises
            // `foundDuplicateFieldInFieldIdLookupModeError` when a requested ID resolves to
            // more than one physical field, and only when that ID is actually requested.
            ...
            // Name-based lookup. Case-sensitive mode uses an exact-match HashMap; on a
            // (pathological) duplicate exact name the last field wins, matching Spark's
            // `caseSensitiveParquetFieldMap` built with `.toMap`. Case-insensitive mode
            // mirrors Spark's actual Parquet footer matching (see
            // `names_equal_ignore_case_java`), which groups physical field names by
            // full-string `toLowerCase(Locale.ROOT)`; like Spark's
            // `caseInsensitiveParquetFieldMap` groupBy, every colliding index is kept and
            // `foundDuplicateFieldInCaseInsensitiveModeError` fires only when a REQUESTED
            // field's lookup turns out ambiguous -- unrequested collisions read fine.
            let exact_name_to_index_map: Option<HashMap<&str, usize>> =
                if parquet_options.case_sensitive {
                    let mut map = HashMap::new();
                    for (i, field) in from_fields.iter().enumerate() {
                        map.insert(field.name().as_str(), i);
                    }
                    Some(map)
                } else {
                    None
                };
            let case_insensitive_map: Option<HashMap<String, Vec<usize>>> =
                if parquet_options.case_sensitive {
                    None
                } else {
                    let mut map: HashMap<String, Vec<usize>> =
                        HashMap::with_capacity(from_fields.len());
                    for (i, field) in from_fields.iter().enumerate() {
                        map.entry(java_lowercase(
                            field.name(),
                            parquet_options.jvm_case_tables.as_deref(),
                        ))
                        .or_default()
                        .push(i);
                    }
            ...
            let from_index_by_name = |name: &str| -> DataFusionResult<Option<usize>> {
                if let Some(map) = &exact_name_to_index_map {
                    return Ok(map.get(name).copied());
                }
                let map = case_insensitive_map
                    .as_ref()
                    .expect("case_insensitive_map is set whenever case_sensitive is false");
                match map.get(&java_lowercase(
                    name,
                    parquet_options.jvm_case_tables.as_deref(),
                )) {
                    None => Ok(None),
                    ...
                    Some(indices) => {
                        ...
                        Err(DataFusionError::External(Box::new(
                            SparkError::DuplicateFieldCaseInsensitive {
                                required_field_name: name.to_string(),
                                matched_fields: format!("[{}]", matched),
                            },
                        )))
                    }
                }
            ...
            for to_field in to_fields.iter() {
                let from_index = match (should_match_by_id, field_id(to_field)) {
                    ...
                    _ => from_index_by_name(to_field.name())?,
                };
...
/// registers object stores under, plus the "is this an HDFS-scheme URL" classification the
/// `s3a` -> `s3` remap below depends on. Pure and I/O-free (no config hashing, no cache lock, no
/// store creation/registration): a caller that keeps its OWN local `ObjectStoreUrl`-keyed cache
/// (e.g. `delta_spark_scan.rs`'s `resolve_store`, which resolves a store per FILE but only needs
/// one per distinct authority) can compute this cheap key first and consult its local cache
/// before ever calling into the expensive resolution path below.
pub(crate) fn object_store_url_key(
    ...
    let scheme = if !is_hdfs_scheme && url.scheme() == "s3a" {
        "s3"
    } else {
        url.scheme()
    };
...
pub(crate) fn prepare_object_store_with_config_hash(
    ...
    if !is_hdfs_scheme && url.scheme() == "s3a" {
...
mod tests {
    ...
    #[test]
    fn struct_cast_does_not_match_capital_i_with_dot_above_to_ascii_i() {
        ...
        // Physical (file) struct field named plain ASCII "I"; logical field named 'İ'
        // (U+0130, LATIN CAPITAL LETTER I WITH DOT ABOVE). Both Java's
        // `toLowerCase(Locale.ROOT)` and Rust's `str::to_lowercase` fold these to DIFFERENT
        // strings ("i" vs "i" + COMBINING DOT ABOVE), matching Spark's actual
        // `toLowerCase`-keyed Parquet footer field matching -- so the logical field should NOT
        // resolve to the physical column and must come back null rather than 42.
        let from_field = Field::new("I", DataType::Int32, true);
        let from_array: ArrayRef = Arc::new(Int32Array::from(vec![Some(42)]));
        let from_struct = StructArray::new(
            Fields::from(vec![from_field.clone()]),
            vec![from_array],
            None,
        );

        let to_field = Field::new("\u{130}", DataType::Int32, true);
        let to_type = DataType::Struct(Fields::from(vec![to_field]));

        let mut parquet_options = SparkParquetOptions::new(EvalMode::Legacy, "UTC", false);
        parquet_options.case_sensitive = false;

        let result = parquet_convert_array(Arc::new(from_struct), &to_type, &parquet_options)
            .expect("struct cast should succeed");
        let result_struct = result.as_any().downcast_ref::<StructArray>().unwrap();
        let result_col = result_struct
            .column(0)
            .as_any()
            .downcast_ref::<Int32Array>()
            .unwrap();
        assert!(
            result_col.is_null(0),
            "expected physical field \"I\" to NOT match logical field \"\\u{{130}}\" and come \
             back null, per Spark's toLowerCase-keyed field matching"
        );
    }
    ...
    mod struct_field_matching {
        ...
        /// The physical struct holds `B` and `b` (a case-insensitive collision) plus `c`,
        /// but only `c` is requested. Spark's `matchCaseInsensitiveField` errors solely when
        /// the REQUESTED field's `toLowerCase(Locale.ROOT)` lookup is ambiguous, so this
        /// read must succeed and return `c`'s value.
        #[test]
        fn unrequested_case_insensitive_duplicate_reads_fine() {
            let from = struct_of(
                vec![
            ...
            let to_type =
                DataType::Struct(Fields::from(vec![Field::new("c", DataType::Int32, true)]));
        ...
        /// Requesting `b` against physical `B`/`b` in case-insensitive mode is ambiguous:
        /// Spark raises `foundDuplicateFieldInCaseInsensitiveModeError`
        /// (`_LEGACY_ERROR_TEMP_2093`).
        #[test]
        fn requested_case_insensitive_duplicate_errors() {
            let from = struct_of(
                vec![
                    Field::new("B", DataType::Int32, true),
                    Field::new("b", DataType::Int32, true),
            ...
            let msg = err.to_string();
            assert!(
                msg.contains("_LEGACY_ERROR_TEMP_2093") && msg.contains('b'),
                "unexpected error: {msg}"
            );
        }
        ...
        /// entry wins silently; the exact-name map here must do the same rather than fail.
        #[test]
