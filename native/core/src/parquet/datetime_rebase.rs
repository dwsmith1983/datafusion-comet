...
impl FileRebasePolicies {
    ...
    /// The policy of the timezone-carrying timestamp leaf at depth-first ordinal `leaf`: its
    /// physical type's spec when the attribution is known; otherwise the two specs merged --
    /// agreement decides, disagreement degrades to [`RebasePolicy::CheckAncient`], which still
    /// passes every modern value and refuses only ancient ones.
...
/// timezone-carrying timestamps have a policy to apply; timezone-free timestamps are
/// `TIMESTAMP_NTZ`, which Spark never rebases, and every other leaf is the identity
/// ([`RebasePolicy::Corrected`]).
fn leaf_policies(
    dt: &DataType,
    next_leaf: &mut usize,
...
/// evaluate rebased values. Columns whose leaves are all the identity -- unaffected types, or
/// affected types under [`RebasePolicy::Corrected`] -- pass through unwrapped. (The pruning
