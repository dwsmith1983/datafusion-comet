...
use crate::parquet::schema_adapter::JvmCaseTables;
use arrow::array::{
...
impl PhysicalPlanner {
    ...
    fn build_parquet_scan_plan(
        ...
        // Case tables shipped by the planning JVM (populated only when case_sensitive is
        // false): they let native's case-insensitive footer matching reproduce that JVM's
        // `toLowerCase(Locale.ROOT)` exactly. Absent tables on a case-insensitive scan (an
        // old plan) degrade to a `str::to_lowercase` fallback in `java_lowercase`.
        let jvm_case_tables = if !common.case_sensitive && !common.jvm_lower_cp.is_empty() {
            Some(Arc::new(JvmCaseTables::from_proto(
                &common.jvm_lower_cp,
                &common.jvm_lower_repl,
                &common.jvm_sigma_class_ranges,
            )))
        } else {
            None
        };
