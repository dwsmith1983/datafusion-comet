...
object CometNativeScan extends CometOperatorSerde[CometScanExec] with CometTypeShim with Logging {
  ...
  private[comet] def populateScanConfFlags(
    ...
    if (!caseSensitive) {
      // Ship THIS JVM's case data so native's case-insensitive footer matching reproduces
      // this JVM's `toLowerCase(Locale.ROOT)` exactly, whatever Unicode version it bundles.
