...
class CometDeltaNativeScanSuite extends CometDeltaTestBase {
  ...
  // planning JVM's `toLowerCase(Locale.ROOT)` from shipped case tables (see
  // `names_equal_ignore_case_java` in schema_adapter.rs and JvmCaseTables.scala), so
  // caseSensitive=false would also read these correctly -- there is no decline gate involved
    ...
    withSQLConf(SQLConf.CASE_SENSITIVE.key -> "false") {
      withTempPath { dir =>
        ...
        withTable(table) {
          ...
          // name matcher (`names_equal_ignore_case_java` in schema_adapter.rs) always matched
          // correctly, ASCII being the easy case.
  ...
  test(
    ...
    withSQLConf(SQLConf.CASE_SENSITIVE.key -> "false") {
      withTempPath { dir =>
        ...
        // matching (`names_equal_ignore_case_java` in schema_adapter.rs) at all.
        spark
          .range(0, 20)
