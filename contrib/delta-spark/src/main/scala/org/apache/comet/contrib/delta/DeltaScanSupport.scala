...
object DeltaScanSupport {
   ...
   * tolerated, not flagged, since such a URI cannot come from a Hadoop-backed source.
   */
  ...
  /**
   * The three-way Hadoop credential-provider-path precheck shared by every `getPassword`-based
   * resolution below, factored out of what used to be [[hadoopLookupPasswordEffective]]'s own
   * body so [[effectiveEncryptionAlgorithm]] can reuse it for its own bucket-tier-only and
   * global-tier-only lookups (see that function's doc for why the algorithm needs its resolution
   * split into separate tiers rather than one combined long-then-short-then-global list). This
   * precheck is a property of the BUCKET alone, independent of which base key or alias list a
   * caller goes on to resolve: Arm A -- an S3A-scoped or bucket-scoped Hadoop credential-provider
   * path configures a provider `Configuration#getPassword` does not consult
   * (`S3AUtils#lookupPassword`/`#lookupBucketSecret` build their own S3A-only provider list per
   * bucket, which this plain-conf-reading gate cannot mirror) -- returns
   * [[UnverifiableProvider]], NO keystore I/O. Arm B -- only the GLOBAL Hadoop provider path is
   * set (which `getPassword` DOES consult) -- returns [[GlobalProviderOnly]]; the caller must
   * wrap its own `getPassword` calls in try/catch, since real keystore I/O only happens once this
   * arm is reached. Arm C -- no provider path is set anywhere -- returns [[NoProvider]], zero-I/O
   * plain conf only.
   */
  ...
  /**
   * `aliases`' effective value under `hadoopConf`/`bucket`, tried in the given order and keeping
   * the first non-empty result -- or `Left(reason)` when the value cannot be safely verified.
   * Dispatches on [[credentialProviderArm]]: [[UnverifiableProvider]] declines outright (zero
   * I/O); [[GlobalProviderOnly]] resolves every alias via `Configuration#getPassword` (real
   * keystore I/O, contained in try/catch: a corrupt/unreadable store must decline this bucket,
   * not abort planning for the whole session); [[NoProvider]] resolves every alias via plain
   * [[plainValue]] reads (zero I/O). Generalizes what used to be
   * [[hadoopLookupPasswordEffective]]'s entire body to an explicit, caller-supplied alias list --
   * [[hadoopLookupPasswordEffective]] is now the long-then-short-then-global special case of
   * this; [[effectiveEncryptionAlgorithm]] is the bucket-tier-only (two aliases) and
   * global-tier-only (one alias) special cases, matching
   * `S3AUtils#lookupBucketSecret`/`#lookupPassword`'s real per-tier `getPassword` calls exactly
   * instead of [[plainValue]]'s JCEKS-blind read that under-declined a keystore-only algorithm.
   *
   * The [[NoProvider]] arm honors [[ClearTextFallbackKey]]: `getPassword` reads plaintext conf
   * ONLY while that flag (default true) holds, so with no provider configured anywhere and the
   * flag off, its effective value is nothing at all -- every caller models a real `getPassword`
   * consumer and gets `Right(None)` there, plaintext notwithstanding. (A key whose real consumer
   * is NOT `getPassword` must not resolve through this function at all, [[proxyGateReason]] reads
   * `fs.s3a.proxy.host` via plain `getTrimmed` like its real consumer instead, see
   * [[S3ProxyHostKey]].) The [[GlobalProviderOnly]] arm delegates to `getPassword` itself, which
   * always applies the flag's real semantics.
   */
   ...
   * itself raises: it recursively expands `${...}` references and throws `IllegalStateException`
   * once expansion recurses past Hadoop's `MAX_SUBST` bound (e.g. a two-key mutual reference
   * cycle between `fs.s3a.aws.credentials.provider` and
   * `fs.s3a.assumed.role.credentials.provider` or any other conf entry). Every provider-CLASS
   * read below goes through this wrapper rather than calling [[shortThenGlobal]] directly: unlike
   * [[s3ConfigDivergenceReason]]'s `s3KeyDivergenceReason`, this chain has no outer try/catch of
   * its own, and previously relied on `s3ConfigDivergenceReason` running first and covering the
   * same keys to mask the exception
   * -- an undocumented, bypassable coupling, since [[providerClassGateReason]] is itself a
   * public, directly-callable entry point. Reuses [[unverifiableValueReason]]'s message shape
   * (names the key and the exception class, never a value).
   */
   ...
   * `None` when every named class is supported (or the key is unset). The Hadoop `${...}`
   * variable-reference precheck this used to run ahead of the class-support check is gone: once
   * `NativeConfig` forwards `Configuration#get`'s substituted value for every entry (same as
   * [[shortThenGlobal]] reads here), a `${...}` reference resolves identically for native and for
   * this check, so it is no longer possible for one to see the literal and the other the expanded
   * class name.
   */
